...
typedef struct walk_sequential_accumulator {
    char paths[64][4096];
    size_t path_lengths[64];
    size_t count;
} walk_sequential_accumulator_t;

typedef struct walk_sequential_fixture {
    bc_allocators_context_t* memory_context;
    char root[128];
    walk_sequential_accumulator_t accumulator;
} walk_sequential_fixture_t;

static bool visit_accumulate_files(const bc_io_walk_entry_t* entry, void* user_data)
{
    walk_sequential_accumulator_t* acc = (walk_sequential_accumulator_t*)user_data;
...
static bool visit_returns_false_after_first_file(const bc_io_walk_entry_t* entry, void* user_data)
{
    walk_sequential_accumulator_t* acc = (walk_sequential_accumulator_t*)user_data;
    if (entry->kind != BC_IO_WALK_ENTRY_FILE) {
        return true;
...
{
    walk_sequential_fixture_t* fixture = test_calloc(1, sizeof(*fixture));
    ...
    snprintf(fixture->root, sizeof(fixture->root), "/tmp/bc_io_walk_seq_test_%d", getpid());
...
{
    walk_sequential_fixture_t* fixture = *state;
    cleanup_tree_recursive(fixture->root);
...
static void test_walk_sequential_rejects_invalid_config(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
    ...
    assert_false(bc_io_walk_sequential(&bad, NULL));
...
static void test_walk_sequential_empty_directory(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
    ...
    assert_true(bc_io_walk_sequential(&config, &stats));
...
static void test_walk_sequential_flat_files(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
...
static void test_walk_sequential_nested_files(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
    char child_dir[256];
...
static void test_walk_sequential_filter_rejects_directory(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
    char keep_dir[256];
    char reject_dir[256];
...
static void test_walk_sequential_nonexistent_root_reports_error(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
...
static void test_walk_sequential_visit_failure_aborts(void** state)
{
    walk_sequential_fixture_t* fixture = *state;
    assert_int_equal(build_tree_with_files(fixture->root, 10), 0);
    ...
    bc_io_walk_config_t config = {
        ...
        .root_length = strlen(fixture->root),
        .main_memory_context = fixture->memory_context,
        .visit = visit_returns_false_after_first_file,
        .visit_user_data = &fixture->accumulator,
    };
    assert_false(bc_io_walk_sequential(&config, NULL));
}

int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_walk_sequential_rejects_invalid_config, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_empty_directory, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_flat_files, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_nested_files, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_filter_rejects_directory, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_nonexistent_root_reports_error, setup, teardown),
        cmocka_unit_test_setup_teardown(test_walk_sequential_visit_failure_aborts, setup, teardown),
    };
